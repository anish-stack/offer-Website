...
const AllListings = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const data = response.data.data; // Assuming response.data directly contains the array

            console.log(data)
            // Filter out listings that are approved by admin
            const filteredListings = data.filter(item => item && item.listing && item.listing.isApprovedByAdmin);

            // Reverse the order of filtered listings
            const reversedListings = filteredListings;
            console.log("reversedListings", reversedListings)

            // Update state or handle the filtered data as needed
